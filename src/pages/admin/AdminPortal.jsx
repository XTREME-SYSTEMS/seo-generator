...
import { KeyRound, Tag, Brain, Users, Plus, Trash2, Copy, Loader2, CheckCircle2, Power, Edit3, X, Shield, Eye, EyeOff } from 'lucide-react';
...
function AdminPortal() {
  ...
  const handleCreateKey = async () => {
    ...
    setCreatingKey(true);
    try {
      const { data } = await base44.functions.invoke('ManageApiKey', {
        action: 'generate', label: newKey.label, level: newKey.level,
      });
  ...
  const handleRevokeKey = async (id) => {
    ...
    try {
      await base44.functions.invoke('ManageApiKey', { action: 'revoke', id });
      loadData();
    } catch (err) { alert('Error: ' + err.message); }
  };
  ...
  const handleUpdateKey = async () => {
    ...
    try {
      await base44.functions.invoke('ManageApiKey', {
        action: 'update', id: editingKey.id, label: editingKey.label, level: editingKey.level,
      });
      setEditingKey(null);
      loadData();
    } catch (err) { alert('Error: ' + err.message); }
  ...
  const handleTogglePromo = async (promoId, active) => {
    try {
      await base44.functions.invoke('StripeCheckout', {
        path: 'toggle-promo', promoId, active: !active,
      });
      loadData();
    } catch (err) { alert('Error: ' + err.message); }
  };
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage API keys, promo codes, and client subscriptions</p>
          </div>
        </div>
      </div>
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: 'Active Subscriptions', value: activeSubs.length, icon: CheckCircle2, color: 'text-emerald-600' },
              { label: 'API Keys', value: apiKeys.filter(k => k.status === 'active').length, icon: KeyRound, color: 'text-primary' },
              { label: 'Promo Codes', value: promoCodes.filter(p => p.active).length, icon: Tag, color: 'text-primary' },
              { label: 'Total Clients', value: subscriptions.length, icon: Users, color: 'text-sky-600' },
            ].map(s => (
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            <h3 className="mb-4 font-heading text-sm font-semibold">Plan Distribution</h3>
            <div className="space-y-2">
              {['starter', 'professional', 'elite', 'enterprise'].map(plan => (
                <div key={plan} className="flex items-center justify-between">
                  <span className="text-sm capitalize text-foreground">{plan}</span>
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-32 overflow-hidden rounded-full bg-muted">
                      <div className="h-full bg-primary" style={{ width: `${activeSubs.length > 0 ? ((planCounts[plan] || 0) / activeSubs.length) * 100 : 0}%` }} />
                    </div>
                    <span className="font-mono text-xs text-muted-foreground">{planCounts[plan] || 0}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Generate keys with different access levels for external integrations</p>
            </div>
            ...
            <div className="mb-4 rounded-lg border border-emerald-300 bg-emerald-50 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mb-2 text-xs text-emerald-700">This is the only time the full key will be shown. Copy it now.</p>
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" onClick={() => copyToClipboard(generatedKey.api_key, 'generated')} className="bg-emerald-600 text-white hover:bg-emerald-700">
                      {copied === 'generated' ? <CheckCircle2 className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                    </Button>
                  </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <select value={newKey.level} onChange={e => setNewKey({ ...newKey, level: e.target.value })} className="w-full rounded border border-input bg-background px-3 py-2 text-sm">
                    ...
                    <option value="write">Read + Write — Can create/update</option>
                    <option value="admin">Full Admin — Can delete + manage</option>
                  </select>
                </div>
              </div>
                  ...
                  {editingKey?.id === key.id ? (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {key.status === 'active' && (
                          <>
                            <button onClick={() => setEditingKey(key)} className="rounded p-1.5 text-muted-foreground hover:bg-muted" title="Edit">
                              <Edit3 className="h-4 w-4" />
                            </button>
                            <button onClick={() => handleRevokeKey(key.id)} className="rounded p-1.5 text-red-400 hover:bg-red-400/10" title="Revoke">
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Create and manage discount codes for checkout</p>
            </div>
                ...
                <div key={p.id} className={`flex items-center justify-between rounded-lg border bg-card px-4 py-3 ${p.active ? 'border-border' : 'border-red-200 opacity-60'}`}>
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {p.percentOff}% off · {p.duration}{p.duration === 'repeating' ? ` (${p.durationInMonths || 1} mo)` : ''} · {p.timesRedeemed || 0} redeemed
                    </p>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => copyToClipboard(p.code, p.id)} className="rounded p-1.5 text-muted-foreground hover:bg-muted" title="Copy code">
                      {copied === p.id ? <CheckCircle2 className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4" />}
                    </button>
                    <button onClick={() => handleTogglePromo(p.id, p.active)} className="rounded p-1.5 text-muted-foreground hover:bg-muted" title={p.active ? 'Deactivate' : 'Activate'}>
              ...
              {subscriptions.map(s => (
                <div key={s.id} className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      <span className="capitalize">{s.plan}</span> plan · {s.url_limit >= 9999 ? 'Unlimited' : s.url_limit} URLs · Onboarding: {s.onboarding_completed ? '✓' : '✗'}
                    </p>
                  </div>
